...
def convert_and_save(template_path: str, output: str):
    tmpl_path = Path(template_path)
    ...
    ###################################################
    p = Path("/function/terraform_src/website/docs")
    # p = Path("/tmp/terraform_src/website/docs")
    search_manger = code.SearchManager(p)
    # search_manger = code.search_manager()
    config = TemplateConverter(tmpl_path.stem, cf_template, search_manger).convert()
...
if __name__ == "__main__":
    # cli()  # type: ignore
    convert_and_save("./input/test.json", "./output")
